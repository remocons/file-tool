...
class File extends buffer.Blob {
...
function renameDirFilesRandomUUID(dirPath) {
  return promises.readdir(dirPath).then(fileList => {
    fileList.forEach(file => {
      if (file[0] !== '.') {
        promises.rename(pathJoin(dirPath, file), pathJoin(dirPath, crypto__default["default"].randomUUID()));
      }
    });
  }).catch(err => {
    console.error(err);
...
function renameRandomUUID(filePath) {
  const dirName = path__default["default"].dirname(filePath);
  return promises.rename(filePath, path__default["default"].join(dirName, crypto__default["default"].randomUUID()))
    .catch(err => {
      console.error(err);
    })
}
...
function readDirFiles(path) {
  return promises.readdir(path).catch(err => console.log(err))
}
function readDirFilesSync(path) {
...
function getBlobFromURL(blobURL) {
  return buffer.resolveObjectURL(blobURL)
}

function loadFileList(filePathList) {
  const blobList = [];
...
function getFileSize(filepath) {
  return promises.stat(filepath).then(st => {
    return st.size
  })
}
...
async function readFileAsBufferSlice(filePath, start, end) {
  let fd = null;
  try {
...
function readFileAsBufferSync(filePath) {
  try {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (fileSize > 0) {
      ...
      const inStream = new stream.Readable({
        read(size) {
          const rand = crypto__default["default"].randomBytes(this.chunksize);
          this.push(rand);
...
Object.defineProperty(exports, 'Blob', {
  enumerable: true,
  get: function () { return buffer.Blob; }
});
exports.File = File;
...
exports.readFileAsBufferSlice = readFileAsBufferSlice;
exports.readFileAsBufferSync = readFileAsBufferSync;
exports.renameDirFilesRandomUUID = renameDirFilesRandomUUID;
exports.renameFile = renameFile;
exports.renameRandomUUID = renameRandomUUID;
exports.revokeObjectURL = revokeObjectURL;
exports.saveBlob = saveBlob;
exports.wipeRandom = wipeRandom;
